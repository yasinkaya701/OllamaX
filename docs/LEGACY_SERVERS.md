...
Bu dizindeki iki dosya, OllamaX Ultra'nın ilk dönemlerinde kullanılan **eski (deprecated)** araçlardır ve artık uygulamanın çalışması için gerekli değildir.

## `proxy_server.js` (Node.js)

Ollama yerel uç noktasına (`localhost:11434`) tarayıcı tabanlı arayüzün CORS kısıtlamalarını aşarak ulaşmasını sağlayan basit bir ters proxy idi. **Artık kullanılmıyor** çünkü OllamaX Ultra, Electron ana süreci üzerinden (`src/main.js`) doğrudan ve güvenli biçimde Ollama ile iletişim kurmaktadır. Ana süreç zaten SSRF koruması (`src/main-security.js`) içerdiği için bu eski proxy'ye gerek yoktur.
